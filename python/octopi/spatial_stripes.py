...
def pixel_color(t, coord, ii, n_pixels):
    ...
    x, y, z = coord
    r = color_utils.cos(x, offset=t / 3, period=0.3, minn=0, maxx=0.9)
    g = color_utils.cos(y, offset=t / -6, period=0.6, minn=0, maxx=0.9)
    b = color_utils.cos(z, offset=t / -3, period=0.9, minn=0, maxx=0.9)
    r, g, b = color_utils.contrast((r, g, b), 0.2, 3)

    # make a moving white dot showing the order of the pixels in the layout file
    spark_ii = (t*30) % n_pixels
    spark_rad = 90
    spark_val = max(0, (spark_rad - color_utils.mod_dist(ii, spark_ii, n_pixels)) / spark_rad)
    spark_val = min(1, spark_val*1)
    r += spark_val
    g -= spark_val
    b += spark_val
